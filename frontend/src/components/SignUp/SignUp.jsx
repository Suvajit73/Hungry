...
const AwesomeToast = ({ message, icon }) => (
    <div className='animate-slide-in fixed bottom-6 right-6 flex items-center bg-gradient-to-br from-amber-500
...
const SignUp = () => {
    ...
    useEffect(() => {
        if (showToast.visible && showToast.message === 'Sign up Successfull') {
            const timer = setTimeout(() => {
                setShowToast({ visible: false, message: '', icon: null }); // ✅ Fix here
                navigate('/login');
            }, 2000);
    ...
    const handleSubmit = async e => {
        e.preventDefault();
        console.log('Sign up faled:', formData);
        try {
