...
const Navebar = () => {
    ...
    // Combine updateing login model and auth status on Location Change
    ...
    const renderDesktopAuthButton = () => {
        return isAuthenticated ? (
            <button onClick={handleLogout} className='px-3 lg:px-4 py-1.5 lg:py-2 bg-gradient-to-br from-amber-500 to-amber-700 text-[#2D1B0E] rounded-2xl font-bold hover:shadow-lg hover:shadow-amber-600/40 transition-all transform hover:scale-[1.02] border-2 border-amber-600/20 flex items-center space-x-2 shadow-md shadow-amber-900/20 text-sm'>
    ...
    return (
        <nav className='bg-[#2D180E] border-b-8 border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 shadow-[0_25px_50px_-12px] font-vibes group/nav overflow-x-hidden'>
            ...
            <div className='max-w-7xl mx-auto px-4 relative'>
                <div className='flex justify-between items-center h-16 md:h-20 lg:h-24'>
                    ...
                    <div className='hidden md:flex items-center space-x-2 md:space-x-1 lg:space-x-4 flex-1 justify-end'>
                        ...
                        <div className=' flex items-center space-x-2 md:space-x-3 lg:space-x-4 ml-3 md:ml-3 lg:ml-6 md:mr-3 lg:mr-4'>
                            <NavLink to='/cart' className='p-2 md:p-2.0 lg:p-3 text-amber-100 rounded-xl transition-all relative border-amber-900/30 hover:border-amber-600/50 group hover:bg-amber-900/20 hover:shadow-lg hover:shadow-amber-500/30 shadow-md shadow-amber-900/20'>
                    ...
                    <div className='md:hidden flex items-center mr-2'>
                        <button className='text-amber-500 hover:text-amber-300 focus:outline-none transition-all p-2 rounded-xl border-2 border-amber-900/30 hover:border-amber-600/50 relative shadow-md shadow-md shadow-amber-900/20 hover:shadow-lg hover:shadow-amber-500/30' onClick={() => setIsOpen(!isOpen)}>
            ...
            {/* Mobile / Table menu..................................................... */}
            {isOpen && (
                <div className='md:hidden bg-[#2D1B0E] border-t-4 border-b-amber-900/40 relative shadow-lg shadow-amber-900/30 w-full'>
                    <div className=' px-4 py-4 space-y-2'>
                        {navLinks.map((link) => (
                            <NavLink key={link.name} to={link.to} onClick={() => setIsOpen(false)} className={({ isActive }) =>
                                `block px-4 py-3 text-sm rounded-xl transition-all items-center ${isActive ? 'bg-amber-600/30 text-amber-400' : 'text-amber-100 hover:bg-amber-600/20'
            ...
            {/* Login model.................................................. */}
            {showLoginModal && (
                <div className='fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-40'>
