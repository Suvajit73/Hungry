...
const Order = () => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          'https://hungry-backend-jomb.onrender.com/api/orders/getall',
          {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.put(`https://hungry-backend-jomb.onrender.com/api/orders/getall/${orderId}`,
        { status: newStatus });
                        ...
                        <div className='space-y-1 max-h-52 overflow-auto'>
                          {order.items.map((itm, idx) => (
                            <div key={idx} className='flex items-center gap-3 p-2 rounded-lg'>
                              <img src={`https://hungry-backend-jomb.onrender.com${itm.item.imageUrl}`}
                              ...
                              <div className='flex-1'>
                                ...
                                <div className='flex items-center gap-2 text-xs text-amber-400/60'>
                                  <span>₹{Number(itm.item.price).toFixed(2)}</span> {/* original code :-  <span>₹{itm.item.price.toFixed(2)}</span>*/}
                                  <span>&dot;</span>
